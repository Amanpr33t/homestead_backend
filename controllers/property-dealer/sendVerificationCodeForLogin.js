...
const sendVerificationCodeForDealerVerification = async (req, res, next) => {
    try {
        ...
        let doesUserExist
        if (email) {
            doesUserExist = await PropertyDealer.countDocuments({ email: req.body.email })
        } else if (contactNumber) {
            doesUserExist = await PropertyDealer.countDocuments({ contactNumber: req.body.contactNumber })
        }
...
const confirmVerificationCodeForDealerVerification = async (req, res, next) => {
    try {
        const {
            email,
            contactNumber,
